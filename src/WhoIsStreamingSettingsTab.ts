...
export class WhoIsStreamingSettingsTab extends PluginSettingTab {
  ...
  async display(): Promise<void> {
    ...
    new Setting(containerEl)
      .setName("Include additional metadata")
      .setDesc("Include runtime, rating, seasons, episodes, etc.")
      .addToggle((toggle) => {
        toggle
          .setValue(this.plugin.settings.includeAdditionalMetadata)
          .onChange(async (value) => {
            this.plugin.settings.includeAdditionalMetadata = value;
            await this.plugin.saveSettings();
          });
      });
    ...
    new Setting(containerEl)
      .setName("Auto-add genre tags")
      .setDesc("Automatically add genre-based tags to notes")
      .addToggle((toggle) => {
        toggle
          .setValue(this.plugin.settings.autoAddGenreTags)
          .onChange(async (value) => {
            this.plugin.settings.autoAddGenreTags = value;
            await this.plugin.saveSettings();
          });
      });
  ...
  async initializeStreamingServices(): Promise<void> {
    ...
    try {
      ...
      // Error initializing streaming services
    }
  }
}
